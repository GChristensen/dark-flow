...
(defn parse-thread-post [doc-tree root-node data target]
  (let [thread-id (get (.split (.getAttribute (pp/select doc-tree "#topic-title") "href") "=") 1)
...
(defmethod pp/parse-threads +trademark+ [doc-tree target]
  (if (pp/exists-in doc-tree "table#topic_main")
    (let [data (pp/cons-post-data :omitted 0
          ...
          replies (loop [replies (take 3 (pp/select* doc-tree "#topic_main tbody[id]")) num 1 out  []]
                        (let [reply (first replies)]
                             (if reply
                               ...
                               out)))]
         [(assoc oppost :children replies :post-count (count replies) :incomplete true)])
