...
function get_pages(cons_xhr, callback, prefix, data)
{
    if (data.payload.length > 0)
    {
        ...
        function check_responses()
        {
            ...
            if (ready)
                callback({state: has_data? "ok": "error",
                    pages: responses});
        }
...
function post_form(cons_xhr, callback, prefix, data)
{
    var req = cons_xhr();
    req.onload = function()
    {
        console.log(prefix + data.payload.url);
...
net.get_pages = get_pages;
net.post_form = post_form;